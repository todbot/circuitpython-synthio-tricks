...
#
# USB MIDI control:
# - note on / off as expected
# - CC 74 - filter cutoff
# - CC 71 - filter resonance
# - CC 72 - amp envelope release time
# - CC 1 - (mod wheel) - pitch vibrato
#
import time,random
import board
...
osc_detune = 0.01      # how much to detune oscillators for phatness
...
vibrato_lfo_hi = 0.5   # vibrato amount when modwheel is maxxed out
...
lfo_vibrato = synthio.LFO(rate=vibrato_rate, scale=0.31) # scale set with modwheel

oscs = []   # holds currently sounding oscillators
filter_freq = 1000  # current setting of filter
...
while True:
    ...
    if isinstance(msg, NoteOn) and msg.velocity != 0:
        ...
        elif msg.control == 72: # env release time
            amp_env_release_time = map_range( msg.value, 0,127, 0.1, 5)
